...
std::ostream& operator<<(std::ostream& stream, std::chrono::duration<double> d) {
    if (d.count() < 0.000001)
...
std::ostream& operator<<(std::ostream& stream, std::chrono::milliseconds d) {
    return stream << d.count() << "ms";
}
std::ostream& operator<<(std::ostream& stream, std::chrono::microseconds d) {
    return stream << d.count() << "us";
}
std::ostream& operator<<(std::ostream& stream, Measure m) {
    return stream << m.total << " \t" << (m.count==0?std::chrono::duration<double>::zero():(m.total/m.count)) << " \t" << m.min << " \t" << m.max;
}
